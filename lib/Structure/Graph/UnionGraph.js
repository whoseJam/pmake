...
export function UnionGraph(node) {
    let self = {};
...
function update() {
    ...
    function realY(y) {
        ...
        let gy = self._.y + dy / ty * self._.height; 
        return self._.y + (self._.y + self._.height - gy);
    }
    ...
    graph.edges().forEach(function(info) {
        ...
        let nx = nodes[x].node;
        let ny = nodes[y].node;
        if (links[encode(x, y)][0].isFirst) {
            self.linkPreIn()(handle);
            handle.from(nx).to(ny);
            self.linkIn()(handle);
            links[encode(x, y)][0].isFirst = false;
