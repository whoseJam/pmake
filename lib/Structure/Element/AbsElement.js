...
function value(value) {
    ...
    if (ovalue) {
        if (this._.preventRemove) ovalue.attachTo(svg());
        if (!this._.preventRemove) ovalue.opacity(0).remove();
        this._.preventRemove = false;
    }
    if (!value) return this;
    value.attachTo(this.layer("value"));
    if (this._.fromExisted) value.startAnimate(this);
    if (!this._.fromExisted) value.opacity(0);
