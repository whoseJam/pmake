...
export function Tree(node) {
    ...
    self.r = r;
    self.update = update;
    self.type = () => { return "Tree"; };
    
    self.width(300);
    self.nodeType(Vertex);
    self.linkType(Link);
    self.g().attr("name", "Tree");
...
export function link(parent, child) {
    ...
    if (arguments.length === 1) {
        ...
        childId = conf.id;
        linkValue = conf.link_value;
        childValue = conf.value;
    }
    if (Traiter.is_slide(parent)) {
        for (let i in nodes)
            if (nodes[i] === parent) parentId = i;
    } else if (Traiter.isText(child)) {
        parentId = parent;
    }
    if (Traiter.is_slide(child)) {
        for (let i in nodes)
    ...
    this.newNode(parentId, childId, childValue);
