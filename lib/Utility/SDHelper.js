import { Text } from "../Node/Basic/Text";
import * as d3 from "d3";

export const SDHelper = {
    forwardFunc: forwardFunc,
    args1Func: args1Func,
    args2Func: args2Func,
    positionFunc: positionFunc,
    keyValueFunc: keyValueFunc,
    keyBoolValueAsTrueFunc: keyBoolValueAsTrueFunc,
    emptyFunc: function() {},
    any2Slide: any2Slide,
    isOpacity: isOpacity,
    isColor: isColor,
    isSlide: isSlide,
    isText: isText,
    opacity: opacity,
    treeHierarchy: treeHierarchy,
    treeInfo: treeInfo,
    encode: encode
}

function encode(x, y) {
    return x + "%%" + y;
}

function forwardFunc(childId, name) {
    return function() {
        let child = this.children.child(childId);
        return child[name].apply(child, arguments);
    }
}

function args1Func(childId, name) {
    return function(value) {
        let child = this.children.child(childId);
        if (value === undefined)
            return child[name]();
        child[name](value);
        return this;
    }
}

function args2Func(childId, name, withUpdate = false) {
    return function(v1, v2) {
        let child = this.children.child(childId);
        if (v1 === undefined)
            return child[name]();
        child[name](v1, v2);
        if (withUpdate) this.update();
        return this;
    }
}

function positionFunc(childId, name) {
    let signals = [];
    for (let i = 2; i < arguments.length; i++)
        signals.push(arguments[i]);
    return function(value) {
        let child = this.children.child(childId);
        if (value === undefined)
            return child[name]();
        child[name](value);
        this.update();
        for (let i = 0; i < signals.length; i++)
            this.call(signals[i]);
        return this;
    }
}

function keyValueFunc(self, key, defaultValue) {
    self._[key] = defaultValue;
    return function(value) {
        if (value === undefined)
            return this._[key];
        this._[key] = value;
        return this;
    }
}

function keyBoolValueAsTrueFunc(self, key, defaultValue) {
    self._[key] = defaultValue;
    return function() {
        this._[key] = true;
        return this;
    }
}

function any2Slide(self, any) {
    if (any === null) return null;
    if (typeof(any) === "function")
        any = value(any);
    else if (typeof(any) === "string" || typeof(any) === "number")
        any = new Text(self, any);
    return any;
}

function isSlide(obj) {
    if (!obj) return false;
    if (typeof(obj.type) === "function" &&
        typeof(obj.startAnimate) === "function") {
        return true;
    }
    return false;
}

function isOpacity(opacity) {
    return (typeof(opacity) === "number" && 0 <= opacity && opacity <= 1);
}

function isColor(str) {
    if (str === undefined)
        return false;
    if (typeof(str) === "object" && 
        typeof(str.main) !== "undefined" && 
        typeof(str.border) !== "undefined")
        return true;
    if (str[0] !== "#") return false;
    for (let i = 1; i < str.length; i++) {
        if ("0" <= str[i] && str[i] <= "9");
        else if ("a" <= str[i] && str[i] <= "f");
        else if ("A" <= str[i] && str[i] <= "F");
        else return false;
    }
    if (4 <= str.length && str.length <= 7) return true;
    return false;
}

function isText(str) {
    return typeof(str) === "string" || typeof(str) === "number";
}

function opacity(opacity) {
    let children = this.children;
    children.forEach((child) => {
        child.opacity(opacity);
    });
    return this;
}

function treeHierarchy() {
    let rt;
    try {
        rt = d3.stratify()
        rt.id(d => d["nodeId"])
        rt.parentId(d => d["parentNodeId"])
        rt = rt(this._.nodes);
    } catch(error) {
        return null;
    }
    return d3.hierarchy(rt);
}

function treeInfo(hierarchy, width, height) {
    if (!hierarchy) return null;
    let tr = d3.tree().size([width, height]);
    return tr(hierarchy);
}