...
export function Index(node, position="t") {
    let index, layoutOnRow = (position === "t" || position === "b");
    index = (layoutOnRow ? new Array(node) : new Stack(node));
    let len = (layoutOnRow ? 20 : 10);
    index[layoutOnRow ? "elementHeight" : "elementWidth"](len);
    ...
    node.childAs(`index_${position}`, index, function(parent, child) {
        let length = layoutOnRow ? "elementWidth" : "elementHeight";
