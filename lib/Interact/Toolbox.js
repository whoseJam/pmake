...
function flex_item(item, width, height) {
...
export function add_color_picker() {
    let input = handle.append("input")
                      .attr("type", "color")
                      .classed("tool-item-color-picker", true)
    flex_item(input, 30, 30);
    return input;
}

export function add_range(l, r, step = 1, width) {
    let input = handle.append("input")
                      .attr("type", "range")
                      .attr("min", l)
                      .attr("max", r)
                      .attr("step", step)
                      .attr("value", l);
    flex_item(input, width, 30);
    return input;
}

export function add_icon_radio(name, icons, values, checked = 0, width) {
