...
export function setSize(svgElement) {
    ...
    svgElement.setAttribute("viewBox", `${x?x:0} ${y?y:0} ${width} ${height}`);
    console.log(`x = ${x?x:0}, y = ${y?y:0}, width = ${width}, height = ${height}`);
...
export function globalUpdate(flush = false) {
    ...
    const flushUpdate = node => {
        ...
        if (opacity === 0) return;
        if (x !== 0 && y !== 0) {
            window.maxX = Math.max(window.maxX, mx);
            window.minY = Math.min(window.minY, y);
            window.maxY = Math.max(window.maxY, my);
            if (window.minX > x) {
                console.log(node.x(), node.y(), node.mx(), node.my(), node, node.g().type());
            }
            window.minX = Math.min(window.minX, x);
        }
