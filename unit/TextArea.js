...
sd.main(TestTextAreaHTMLAndSVG);

async function TestTextAreaHTMLAndSVG() {
    const t1 = new sd.TextArea(div).x(100).y(100);
    const t2 = new sd.TextArea(svg).x(400).y(100);
}
...
async function TestBasicTextAreaFunctionality() {
    ...
    console.assert(textarea.x() === 100 && textarea.y() === 100 && textarea.width() === 150 && textarea.height() === 100, "Position and size setting test failed");
    await sd.pause();
    textarea.value("测试文本");
...
async function TestTextAreaAnimation() {
    const textarea = new sd.TextArea(div);
    await sd.pause();
    textarea.startAnimate().x(200).y(200).width(150).height(100).value("动画文本框").endAnimate();
...
async function TestTextAreaChaining() {
    ...
    console.assert(textarea.x() === 150 && textarea.y() === 150 && textarea.width() === 150 && textarea.height() === 100 && textarea.value() === "链式调用", "Method chaining test failed");
