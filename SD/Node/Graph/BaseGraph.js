...
BaseGraph.prototype.findLinkById = function (sourceId, targetId) {
    ...
    for (let i = 0; i < links.length; i++) {
        if (sidToLinks[links[i].id].source === sourceGraphId && sidToLinks[links[i].id].target === targetGraphId) {
...
BaseGraph.prototype.inLinks = function (id, mode = "direct") {
    ...
    const links = this.vars.links;
    const sidToLinks = this._.sidToLinks;
    const filter = mode === "direct" ? link => sidToLinks[link.id].target === graphId : link => sidToLinks[link.id].target === graphId || sidToLinks[link.id].source === graphId;
...
BaseGraph.prototype.outLinks = function (id, mode = "direct") {
    ...
    const links = this.vars.links;
    const sidToLinks = this._.sidToLinks;
    const filter = mode === "direct" ? link => sidToLinks[link.id].source === graphId : link => sidToLinks[link.id].target === graphId || sidToLinks[link.id].source === graphId;
    return links.filter(filter);
};
...
BaseGraph.prototype.forEachNode = function (callback) {
    this.nodes().forEach(node => callback(node, this.nodeId(node)));
    return this;
};

BaseGraph.prototype.forEachLink = function (callback) {
