...
Coord.prototype.draw = function(name, func) {
    ...
    for (let key in COORD_LOCATION_KEYS)
        path.member.new(COORD_LOCATION_KEYS[key]);
    ...
    path.trimCoordX = function(y) { return Math.min(Math.max(this.coordX(y), parent.viewX(), parent.viewX() + parent.viewWidth())); }
    path.trimCoordY = function(x) { return Math.min(Math.max(this.coordY(x), parent.viewY(), parent.viewY() + parent.viewHeight())); }
    path.globalY = function(x) { return parent.globalY(this.coordY(x)); }
    path.globalX = function(y) { return parent.globalX(this.coordX(y)); }
