...
async function erfen(mid) {
    ...
    const bT = new sd.Brace(svg).source(grid.x(), grid.y() - 5).target(grid.mx(), grid.y() - 5).value("T", R.PointAtPathByRate(0.5, "cx", "my"));
    const bMid = new sd.Brace(svg).target(grid.x() - 5, grid.y()).source(grid.x() - 5, grid.my()).value("mid", R.PointAtPathByRate(0.5, "mx", "cy", -5));
