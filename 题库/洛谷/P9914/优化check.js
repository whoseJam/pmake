...
const graph = new sd.GridGraph(svg).n(n).m(m).height(n * 70).width(m * 70);

sd.init(() => {
    arrowA.source(graph.x(), graph.my()).target(graph.mx() + 50, graph.my()).arrow();
    arrowB.source(graph.x(), graph.my()).target(graph.x(), graph.y() - 50).arrow();
