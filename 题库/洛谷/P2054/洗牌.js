...
sd.main(async () => {
    await sd.pause();
    new sd.Line(svg).stroke(C.red).strokeWidth(3).source(cards.pos("cx", "y")).target(cards.pos("cx", "my")).startAnimate().pointStoT().endAnimate();
    for (let i = 1; i <= n / 2; i++) {
        await sd.pause();
        ...
        sd.Link(cards.element(i + n / 2), newCard1, sd.Line, "cx", "my", "cx", "y")
            ...
            .endAnimate()
            .arrow();
        sd.Link(cards.element(i), newCard2, sd.Line, "cx", "my", "cx", "y").opacity(0).after(300).opacity(1).startAnimate().pointStoT().endAnimate().arrow();
    }
});
