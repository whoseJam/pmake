...
async function Solve(arr, gap) {
    if (arr.length() === 1) {
        ...
        afterAll.push(() => {
            ...
            if (t1 === "000") {
                ...
                t3.mx(lb1.x() - 20).y(lb1.y()).opacity(0).startAnimate().opacity(1).endAnimate();
                t4.mx(lb2.x() - 20).y(lb2.y()).opacity(0).startAnimate().opacity(1).endAnimate();
            }
        });
        return;
    }
    ...
    await sd.pause();
    f0.startAnimate().freeze();
    f1.startAnimate().freeze();
    for (let i = 0; i < n; i++) {
    ...
    f0.unfreeze().endAnimate();
    f1.unfreeze().endAnimate();
    sd.Link(arr, f0, sd.Line, "cx", "my", "cx", "y").startAnimate().pointStoT().endAnimate().arrow();
    sd.Link(arr, f1, sd.Line, "cx", "my", "cx", "y").startAnimate().pointStoT().endAnimate().arrow();
