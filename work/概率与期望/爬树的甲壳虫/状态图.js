...
sd.init(() => {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        if (i < n) {
            ...
            c2r.value(new sd.Mathjax(c2r, `P_{${i + 1}}`).fontSize(10), R.pointAtPathByRate(0.5, "cx", "y", 0, 3));
        }
        const c2n = sd.Link(prev, current, sd.Curve).bending(-0.5).arrow();
        c2n.value(new sd.Mathjax(c2n, `1-P_{${i}}`).fontSize(10), R.pointAtPathByRate(0.5, "cx", "my", 0, -3));
    }
    const root = arr.element(0);
    const c2r = new sd.CircleCurve(svg).revArrow();
    c2r.source([root.kx(0.3), root.y()]);
    c2r.target([root.kx(0.7), root.y()]);
    c2r.value(new sd.Mathjax(c2r, `P_1`).fontSize(10), R.pointAtPathByRate(0.5, "cx", "my", 0, -3));
});
