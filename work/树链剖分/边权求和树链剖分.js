...
function makeTree() {
    ...
    async function climbOnTree(x, y, label) {
        ...
        if (pos[x] + 1 <= pos[y]) {
            await sd.pause();
            markOnTree(y, sn[x], C.orange);
            markOnSeq(pos[x] + 1, pos[y], C.orange, label);
            let l1 = linkTo(tr.element(sn[x]), arr.element(pos[x] + 1), "pos[son[x]]或pos[x]+1");
            let l2 = linkTo(tr.element(y), arr.element(pos[y]), "pos[y]");
            await sd.pause();            
            markOnTree(y, sn[x], C.white);
            markOnSeq(pos[x] + 1, pos[y], C.white);
            l1.startAnimate().remove();
            l2.startAnimate().remove();
            await sd.pause();
