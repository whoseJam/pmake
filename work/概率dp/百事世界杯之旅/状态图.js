...
sd.init(() => {
    ...
    for (let i = 0; i < n; i++) {
        ...
        c2c.value(new sd.Mathjax(c2c, `\\frac{${i}}{${n}}`).fontSize(10), R.pointAtPathByRate(0.5, "cx", "y", 0, 3));
        const c2n = sd.Link(current, next, sd.Curve).bending(-0.5).arrow();
        c2n.value(new sd.Mathjax(c2n, `\\frac{${n - i}}{${n}}`).fontSize(10), R.pointAtPathByRate(0.5, "cx", "my", 0, -3));
    }
});
