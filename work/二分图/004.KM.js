...
for (let i = 0; i < edges.length; i++) {
    ...
    e.valueRule(function(parent, child) {
        let y = g.y() + 60;
    ...
    e.value(sd.Text(e, edges[i][2]).fontSize(20));
    to[edges[i][0]].push({
        to: edges[i][1],
        value: edges[i][2]
    });
...
for (let i = 1; i <= 5; i++) {
    ...
    g.element(u).children.push("Lx", sd.Text(svg, mx), function(parent, child) {
        child.cx(parent.cx()).cy(parent.cy() - 30);
    })
    g.element(v).children.push("Ly", sd.Text(svg, 0), function(parent, child) {
        child.cx(parent.cx()).cy(parent.cy() + 30);
    });
}
