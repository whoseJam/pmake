...
async function show(pos, i) {
    if (pos + (1<<i) - 1 <= n) {
        ...
        arr1.mx(cx - 20).y(100).opacity(0).startAnimate().opacity(1).endAnimate();
        arr2.x(cx + 20).y(100).opacity(0).startAnimate().opacity(1).endAnimate();
        const l1 = sd.Link(arr.element(l + (1<<i-1) - 1), arr1, sd.Line, "mx", "cy").startAnimate().pointStoT().endAnimate().arrow();
        const l2 = sd.Link(arr.element(l + (1<<i-1)), arr2, sd.Line, "x", "cy").startAnimate().pointStoT().endAnimate().arrow();
