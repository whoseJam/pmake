...
sd.main(async () => {
    ...
    await sd.pause();
    math.startAnimate().transformMath("{f_i}={f_{j_0}}-{(-v_i)}{v_{j_0}}", {1:1,2:2,4:4,5:5}).endAnimate();
    await sd.pause();
    ...
    data.forEach((item, idx) => {
        item.circle = new sd.Circle(coord).r(2).color(C.black).center(coord.at(item.x, item.y)).strokeWidth(0).childAs(
            new sd.Mathjax(coord, `(v_{${idx+1}},f_{${idx+1}})`).fontSize(8),
            R.Aside("tc", 2)
        );
    });
    await sd.pause();
    line.source([0, 0]).target(400, -80).cx(coord.cx()).my(coord.my()).startAnimate().opacity(1).endAnimate();
    line.childAs(new sd.Mathjax(line, "k=v_i").fontSize(8), R.PointAtPathByRate(1, "cx", "my"));
