...
sd.init(() => {
    ...
    sd.Brace(grid).brace(grid.element(b + 1, a + 1), grid.element(b + 1, a + c), "t").value("c");
    sd.Brace(grid).brace(grid.element(b + 1, a + c), grid.element(b + d, a + c), "r").value("d");
    ...
    grid.forEachElement((element, rowId, colId) => {
        ...
        element.onClick(() => {
            sd.inter(async () => {
                ...
                if (flg === 1) grid.element(rowId, colId).after(0).startAnimate().value(new sd.Circle(svg).color(C.orange).r(15), R.centerOnly()).endAnimate();
