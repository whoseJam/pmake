...
export class Game extends sd.SD2DNode {
        ...
        const cardCallback = () => {
            ...
            if (this._.focusAt === undefined) {
                ...
                sd.inter(async () => {
                    ...
                    if (stack.length() >= 2 && stack.lastElement().color().fill === stack.element(stack.end() - 1).color().fill) stack.startAnimate().erase(stack.end()).erase(stack.end()).endAnimate();
            ...
            label.onClick(() => {
                ...
                if (this._.focusAt === undefined) {
                    ...
                    if (this._.focusAt.type === "stack") {
                        ...
                        sd.inter(async () => {
                            focus.startAnimate().focus(null).endAnimate();
                            if (otherStack.length() > 0 && stack.length() > 0 && otherStack.firstElement().color().fill === stack.firstElement().color().fill) {
    ...
    moveTo(stackId) {
        ...
        if (stack.length() >= 2 && stack.lastElement().color().fill === stack.element(stack.end() - 1).color().fill) stack.erase(stack.end()).erase(stack.end());
        if (this.lastCard()) this.lastCard().onClick(cardCallback);
        return this;
    }
    lastCard() {
