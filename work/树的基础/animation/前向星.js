...
const parent = new sd.Input(stk).label("parent");
const child = new sd.Input(stk).label("child");
const button = new sd.Button(stk).text("连边");
button.onClick(() => {
    fw.link(+parent.value(), +child.value());
})
stk.push(parent).push(child).push(button);
stk.x(1000).y(400);
