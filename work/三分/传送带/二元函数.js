...
async function update(first = false) {
    ...
    const p4 = lCD.target();
    if (!first) path.startAnimate().d(new sd.PathPen().MoveTo(p1).LinkTo(p2).LinkTo(p3).LinkTo(p4).toString()).endAnimate();
    else path.d(new sd.PathPen().MoveTo(p1).LinkTo(p2).LinkTo(p3).LinkTo(p4).toString()).stroke(C.red).strokeWidth(3);
}