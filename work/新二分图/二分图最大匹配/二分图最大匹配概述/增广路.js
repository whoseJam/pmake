...
async function AugmentingPath(path) {
    ...
    for (let i = 1; i < path.length; i++) {
        pen.LinkTo(V.add(bi.element(path[i]).center(), [5, 0]));
    }
    const d = new sd.Path(svg).d(pen.toString()).stroke(C.textBlue).strokeWidth(2);
    d.startAnimate(300 * (path.length - 1)).pointStoT().endAnimate().arrow();
    ...
    for (let i = 0; i < path.length - 1; i++) {
        ...
        const l = new sd.Line(tree).strokeWidth(l0.strokeWidth()).stroke(l0.stroke()).source(l0.source()).target(l0.target());
        tree.newLinkFromExistElement(u, v, l);
    }
    tree.unfreeze().cx(bi.cx()).y(bi.my() + 20).endAnimate();

    await sd.pause();
    d.startAnimate(300 * (path.length - 1)).fadeStoT().endAnimate().remove();
