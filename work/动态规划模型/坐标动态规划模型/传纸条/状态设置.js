...
sd.main(async () => {
    await sd.pause();
    const m1 = new sd.Mathjax(svg, "x_1,y_1").fontSize(10).center(grid.element(x1, y1).center()).opacity(0).startAnimate().opacity(1).endAnimate();
    const m2 = new sd.Mathjax(svg, "x_2,y_2").fontSize(10).center(grid.element(x2, y2).center()).opacity(0).startAnimate().opacity(1).endAnimate();
