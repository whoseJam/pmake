...
const arr1 = new sd.BarArray(svg);
...
sd.main(async () => {
    ...
    for (let i = 0; i < arr2.length(); i++) {
        ...
        while (arr1.length() >= 2 && arr1.intValue(arr1.end()) < arr1.intValue(arr1.end() - 1)) {
            await sd.pause();
            arr1.startAnimate().erase(arr2.end() - 1).endAnimate();
        }
        await sd.pause();
        arr1.startAnimate().color(arr1.end(), C.white).endAnimate();
        const lastIndex = arr1.length() >= 2 ? arr1.element(arr1.end() - 1).idx : -1;
        if (lastIndex !== -1) {
            sd.Link(arr2.element(i), arr2.element(lastIndex)).startAnimate().pointStoT().endAnimate();
        }
    }
})