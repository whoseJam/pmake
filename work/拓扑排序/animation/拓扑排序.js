...
export async function toposort(graph, Qgap, callback) {
    ...
    nodes.forEach(node => { node.ind = 0 });
    links.forEach(link => { 
        graph.findNodeById(link.toNodeId).ind++;
    });
    const Q = new sd.Array(graph);
    const SEQ = new sd.Array(graph);
    maxIndgree = Math.max.apply(null, nodes.map(node => node.ind));
    ...
    while (Q.length() > 0) {
        ...
        for (let i = 0; i < outLinks.length; i++) {
            ...
            if (!(v.ind)) {
                Q.after(v).startAnimate().push(v.nodeId).endAnimate();
            }
        }
        await sd.pause();
