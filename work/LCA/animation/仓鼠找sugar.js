import * as sd from "@/sd";

const svg = sd.svg();
const C = sd.color();
const tree = new sd.Tree(svg).width(700);
const n = 12;
const links = [
    [1, 2], [1, 3], [1, 4],
    [2, 5], [2, 6],
    [3, 7], [3, 8], [3, 9],
    [4, 10], [5, 11], [6, 12]
];

init();
main();

function init() {
    links.forEach(link => tree.link(link[0], link[1]));
}

async function main() {
    await sd.pause();
    const nodes = unique(4);
    drawPath(nodes[0], nodes[1], C.deepSkyBlue);
    drawPath(nodes[2], nodes[3], C.red);
    await sd.pause();
}

function unique(count) {
    const ans = [];
    function contains(x) {
        for (let i = 0; i < ans.length; i++)
            if (ans[i] === x) return true;
        return false;
    }
    for (let i = 1; i <= count; i++) {
        while (true) {
            const u = sd.rand(1, n);
            if (!contains(u)) {
                ans.push(u);
                break;
            }
        }
    }
    return ans;
}

function drawPath(u, v, col) {
    while (u !== v) {
        if (tree.depth(u) < tree.depth(v)) {
            let tmp = u; u = v; v = tmp;
        }
        const fa = tree.father(u).nodeId;
        drawLink(fa, u, col);
        u = fa;
        console.log("u=", u, "v=", v);
    }
}

function drawLink(u, v, col) {
    const nodeU = tree.element(u);
    const nodeV = tree.element(v);
    const dy = (col === C.red) ? -2 : 2;
    
    const l = new sd.Line(svg)
        .source([nodeU.cx(), nodeU.cy() + dy])
        .target([nodeV.cx(), nodeV.cy() + dy])
        .strokeWidth(3).stroke(col)
        .opacity(0).startAnimate().opacity(1).endAnimate();
}