    ...
    self.dp = async function() {
        for (let i = 1; i <= n; i++) {
            ...
            for (let j = 1; j <= m; j++) {
                ...
                let t = new sd.HorizontalValueTree(svg).layerWidth(300), ans = 0, tmp;
                t.root(-1, tmp=makeBackpack(j, C.blue));
                sd.MathjaxLabel(tmp, `F_{${i}, ${j}}`, "tc");
                dp.startAnimate();
                for (let k = 0; k <= s[i] && j >= v[i] * k; k++) {
                ...
                dp.endAnimate();
                t.x(600).y(100).height(400).opacity(0).startAnimate().opacity(1).endAnimate();
